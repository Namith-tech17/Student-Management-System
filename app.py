...
def import_students():
    ...
    try:
        with open(
            os.path.join(os.path.dirname(__file__), 'students.csv'),
            'r'
        ) as file:
...
def dashboard():
    ...
    for s in students:
        ...
        student_data.append(
            (student_id, name, usn, percent, risk)
        )
...
def absent(id, date):
    ...
    if existing:
        ...
        cursor.execute(
            "UPDATE attendance SET status='Absent' WHERE student_id=? AND date=?",
            (id, date)
        )
...
def present(id, date):
    ...
    if existing:
        ...
        cursor.execute(
            "UPDATE attendance SET status='Present' WHERE student_id=? AND date=?",
            (id, date)
        )
